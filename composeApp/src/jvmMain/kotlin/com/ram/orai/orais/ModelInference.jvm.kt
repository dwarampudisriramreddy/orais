...
/**
 * Desktop JVM ModelInference implementation for Conditions detection using ONNX Runtime
 * Model: yolov8_640_float32.onnx (dental condition classifier/detector)
 * Input: float32[1, 640, 640, 3]
 * Output: float32[1, 300, 6] - [x_center, y_center, width, height, confidence, class_id]
 */
actual class ModelInference {
    private var session: OrtSession? = null
    private val env = OrtEnvironment.getEnvironment()
    private val conditionLabels = mutableListOf<String>()
    private var debugPrinted = false

    actual fun loadModel(modelPath: String): Boolean {
        return try {
            val modelBytes = this::class.java.classLoader.getResourceAsStream(modelPath)?.readBytes()
            if (modelBytes == null) {
                println("Failed to load model: $modelPath not found in resources")
                return false
            }
            ...
            println("Successfully loaded Conditions ONNX model: $modelPath")
            true
        } catch (e: Exception) {
    ...
    private fun loadConditionLabels() {
        try {
            val labelsText = this::class.java.classLoader.getResourceAsStream("labels_conditions.txt")?.bufferedReader()?.readText()
            if (labelsText != null) {
                conditionLabels.clear()
                conditionLabels.addAll(labelsText.lines().filter { it.isNotBlank() })
                println("Loaded ${conditionLabels.size} condition labels")
            }
        } catch (e: Exception) {
            println("Error loading condition labels: ${e.message}")
        }
    }
    ...
    private fun parseProposal(
        ...
        if (confidence >= confThreshold) {
            ...
            // Keep coordinates normalized (0-1) - let the UI scale them to screen size
    ...
    fun getConditionLabel(classId: Int): String {
        return if (classId in conditionLabels.indices) {
            conditionLabels[classId]
        } else {
            "Unknown"
        }
    }
